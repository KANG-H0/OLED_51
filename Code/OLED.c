...
/**
 * @function: void OLED_LocalFill(uchar x0, uchar y0, uchar x1, uchar y1, uchar LocalFill)
 * @description: 局部写相同数据(主要用于局部清屏/亮屏)
 * @param {uchar x0, uchar y0, uchar x1, uchar y1, uchar LocalFill，显示起始和结束坐标，写入数据} 
 * @return: void
 */
#if 0
 ...
 * @return: void
 */
void OLED_DisplayChar(uchar x, uchar y, uchar DisplayChar, uchar TextSize)
{
    uchar c = 0, i;
 ...
 * @return: void
 */
void OLED_DisplayStr(uchar x, uchar y, uchar *DisplayStr, uchar TextSize) //(uchar *DisplayStr)改成(uchar DisplayStr[])也可
{
    uchar j = 0;
    while (DisplayStr[j] != '\0') //判断字符串是否显示完成
    {
        OLED_DisplayChar(x, y, DisplayStr[j], TextSize);
        if (TextSize == 1) //6X8的字体列加6，显示下一个字符
            x += 6;
        else if (TextSize == 2) //8X16的字体列加8，显示下一个字符
            x += 8;

        if (x > 122 && TextSize == 1) //TextSize6x8如果一行不够显示了，从下一行继续显示
        {
            x = 0;
            y++;
        }
        if (x > 120 && TextSize == 2) //TextSize8x16如果一行不够显示了，从下一行继续显示
        {
            x = 0;
            y++;
        }
...
/**
 * @function: void OLED_DisplayCN(uchar x, uchar y, uchar Num)
 * @description: 在OLED12864特定位置显示16X16汉字
 * @param {uchar x列, uchar y行, uchar No汉字库中汉字编号} 
 * @return: void
 */
